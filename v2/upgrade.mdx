...
Custom Fields v2 introduces improved field type management. If you have custom field types or specific field configurations, review your `config/custom-fields.php`:
...
Custom Fields v2 now uses trait-based composition for field types instead of abstract base classes. If you have custom field types:

- **Interface**: Your field types should implement `FieldTypeDefinitionInterface` (unchanged)
- **Traits**: Use `HasCommonFieldProperties` and optionally `HasImportExportDefaults` 
- **Benefits**: More flexible composition, easier to extend with new behaviors

```php
class CustomFieldType implements FieldTypeDefinitionInterface
{
    use HasCommonFieldProperties; // Gets sensible defaults
    use HasImportExportDefaults;  // Optional: for import/export
