...
Custom Fields v2 now uses `BaseFieldType` with `FieldSchema` for cleaner field type definitions. If you have custom field types, update them:
...
CustomFields::form()
    ...
    ->withoutSections() // Render fields without section wrappers
    ->build()
...
CustomFields::infolist()
    ...
    // OR
    ->values() // Returns collection of components (requires forModel())
```

<Note>
When using `values()` method, you must call `forModel()` first. The `forSchema()` method is only available with `build()`.
</Note>
